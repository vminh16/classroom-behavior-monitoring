...
class DataCollector(object):
  ...
  """
    ...
    def append(self, frameid, Result):
        ...
        id_to_skeleton_action = {}
        if isinstance(skeleton_action_res, dict):
            for track_id, action_res in skeleton_action_res.items():
                try:
                    id_to_skeleton_action[int(track_id)] = action_res
                except Exception:
                    continue
        elif isinstance(skeleton_action_res, list):
            for item in skeleton_action_res:
                try:
                    track_id, action_res = item
                except Exception:
                    continue
                try:
                    id_to_skeleton_action[int(track_id)] = action_res
        ...
        for idx, mot_item in enumerate(rects):
            ...
            if ids in id_to_skeleton_action:
                self.collector[ids]["skeleton_action"].append(
                    id_to_skeleton_action[ids])
            else:
