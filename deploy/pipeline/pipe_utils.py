...
def parse_mot_res(input):
    ...
    # Handle both multi-class dict outputs (ByteTrack) and single-class lists (OCSORT/BOTSORT)
    if isinstance(boxes, dict):
        iterable = boxes.items()
    else:
        iterable = [(0, boxes[0])]
        scores = {0: scores[0]} if not isinstance(scores, dict) else scores
        ids = {0: ids[0]} if not isinstance(ids, dict) else ids

    for cls_id, cls_boxes in iterable:
        cls_scores = scores.get(cls_id, [])
        cls_ids = ids.get(cls_id, [])
        for box, score, track_id in zip(cls_boxes, cls_scores, cls_ids):
            xmin, ymin, w, h = box
            res = [track_id, cls_id, score, xmin, ymin, xmin + w, ymin + h]
            mot_res.append(res)
...
def refine_keypoint_relative(kpts, bbox):
    """
        Convert keypoints to relative coordinates within bbox (0-1 range).
        Used for custom STGCN model that expects normalized coordinates.
    """
    tl = bbox[:, 0:2]  # (T, 2)
    wh = bbox[:, 2:] - tl  # (T, 2)
    tl = np.expand_dims(np.transpose(tl, (1, 0)), (2, 3))  # (2, T, 1, 1)
    wh = np.expand_dims(np.transpose(wh, (1, 0)), (2, 3))  # (2, T, 1, 1)
    # Relative coords: (kpts - bbox_topleft) / bbox_size -> range [0, 1]
    res = (kpts - tl) / np.maximum(wh, 1e-6)
    return res


def refine_keypoint_local(kpts, bbox):
    """
        Convert keypoints to local coordinates within bbox (pixel coords relative to bbox topleft).
        This matches the training data format where video clips are already cropped.
        
        Training: cropped video 218x266 -> TinyPose -> keypoints like (109, 50)
        Pipeline: full frame -> crop -> TinyPose -> translate to full frame coords -> need to convert back to local
        
        Result: keypoints in pixel space relative to bbox top-left, similar to training cropped videos.
    """
    tl = bbox[:, 0:2]  # (T, 2) - top-left coordinates
    tl = np.expand_dims(np.transpose(tl, (1, 0)), (2, 3))  # (2, T, 1, 1)
    # Local coords: kpts - bbox_topleft (still in pixel space, not normalized)
    res = kpts - tl
    return res
...
def parse_mot_keypoint_relative(input):
    """Parse keypoints with local coordinates (pixel coords relative to bbox topleft) for custom STGCN.
    
    This matches the training data format where TinyPose runs on cropped person videos.
    """
    parsed_skeleton_with_mot = {}
    ids = []
    skeleton = []
    for tracker_id, kpt_seq in input:
        ...
        kpts = np.expand_dims(np.transpose(kpts, [2, 0, 1]),
                              -1)  #T, K, C -> C, T, K, 1
        bbox = np.array(kpt_seq.bboxes, dtype=np.float32)
        # Use local pixel coordinates (relative to bbox top-left) instead of normalized
        skeleton.append(refine_keypoint_local(kpts, bbox))
    parsed_skeleton_with_mot["mot_id"] = ids
    parsed_skeleton_with_mot["skeleton"] = skeleton
    return parsed_skeleton_with_mot
