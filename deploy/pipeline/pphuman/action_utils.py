...
class ActionVisualHelper(object):
    def __init__(self, frame_life=20):
        ...
        self.action_history = {}
        # action_class_to_detect: which class triggers the action display
        # For sleeping detection: class 0 = normal, class 1 = sleeping
        self.action_class_to_detect = 1  # Detect sleeping (class 1)
    ...
    def check_detected(self):
        ...
        for mot_id in self.action_history:
            ...
            if int(self.action_history[mot_id]["class"]) == self.action_class_to_detect:
    ...
    def update(self, action_res_list):
        for mot_id, action_res in action_res_list:
            if mot_id in self.action_history:
                if int(action_res["class"]) != self.action_class_to_detect and int(self.action_history[
                        mot_id]["class"]) == self.action_class_to_detect:
                    continue
