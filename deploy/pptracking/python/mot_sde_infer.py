...
class SDE_Detector(Detector):
    ...
    def postprocess(self, inputs, result):
        ...
        if np_boxes_num[0] <= 0:
            _safe_print('[WARNNING] No object detected.')
    ...
    def predict_image(self,
        ...
        for frame_id, img_file in enumerate(image_list):
            if self.do_mtmct:
                if frame_id % 10 == 0:
                    _safe_print('Tracking frame: %d' % (frame_id))
            ...
            if visual:
                if len(image_list) > 1 and frame_id % 10 == 0:
                    _safe_print('Tracking frame {}'.format(frame_id))
                frame, _ = decode_image(img_file, {})
    ...
    def predict_video(self, video_file, camera_id):
        ...
        _safe_print("fps: %d, frame_count: %d" % (fps, frame_count))
        ...
        if self.save_mot_txts:
            ...
            for line in records:
                f.write(line)
            _safe_print('Flow statistic save in {}'.format(result_filename))
            f.close()
    ...
    def predict_mtmct(self, mtmct_dir, mtmct_cfg):
        ...
        for seq in sorted(seqs):
            ...
            if os.path.isfile(fpath) and _is_valid_video(fpath):
                seq = seq.split('.')[-2]
                _safe_print('ffmpeg processing of video {}'.format(fpath))
                frames_path = video2frames(
                    video_path=fpath, outpath=mtmct_dir, frame_rate=25)
                fpath = os.path.join(mtmct_dir, seq)

            if os.path.isdir(fpath) == False:
                _safe_print('{} is not a image folder.'.format(fpath))
                continue
            ...
            _safe_print('start tracking seq: {}'.format(seq))
