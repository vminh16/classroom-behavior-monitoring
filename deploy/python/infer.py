...
from preprocess import preprocess, Resize, ResizeImage, CropImage, NormalizeImage, Permute, PadStride, LetterBoxResize, WarpAffine, Pad, decode_image, CULaneResize
    ...
    def predict_image(self,
        ...
        for i in range(batch_loop_cnt):
            ...
            if run_benchmark:
                ...
                self.det_times.postprocess_time_s.end()
                self.det_times.img_num += len(batch_image_list)
                # ===== FILTER PERSON ONLY (SAFE) =====
                # Disabled to allow all class_ids to be predicted/visualized.
                # if 'boxes' in result and result['boxes'] is not None:
                #     boxes = result['boxes']
                #     if len(boxes) > 0:
                #         boxes = boxes[(boxes[:, 0] == 0) & (boxes[:, 1] >= self.threshold)]
                #         result['boxes'] = boxes
                #         result['boxes_num'] = np.array([len(boxes)], dtype=np.int32)
                # ====================================
...
def load_predictor(model_dir,
    ...
    """
    runtime_flags = globals().get('FLAGS', None)
    collect_trt_shape_info = bool(
        getattr(runtime_flags, 'collect_trt_shape_info', False))
    tuned_trt_shape_file = getattr(runtime_flags, 'tuned_trt_shape_file', None)
    ...
    if run_mode in precision_map.keys():
        ...
        if collect_trt_shape_info and tuned_trt_shape_file:
            config.collect_shape_range_info(tuned_trt_shape_file)
        elif tuned_trt_shape_file and os.path.exists(tuned_trt_shape_file):
