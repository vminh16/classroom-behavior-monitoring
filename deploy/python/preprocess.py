...
class ResizeImage(object):
    """Resize image by short side, keep ratio."""

    def __init__(self, resize_short):
    ...
    def __call__(self, im, im_info):
        ...
        if type(im) == np.ndarray:
            ...
            im = np.asarray(im.resize((w, h), Image.BILINEAR))

        im_info['im_shape'] = np.array(im.shape[:2]).astype('float32')
        im_info['scale_factor'] = np.array([percent, percent]).astype('float32')
        return im, im_info
